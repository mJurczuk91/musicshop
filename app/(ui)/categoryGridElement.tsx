    ...
    return <div style={{}} className="flex justify-between p-2 border-gray-400 border-opacity-25 odd:text-right md:odd:text-left">
        <div className="flex flex-col w-full">
            ...
            {category.subcategories.slice(0,3).map(subcategory => {
                return <Link key={subcategory.id} className="tracking-tight capitalize" href={`/category/${getCategorySlug(category.name, category.id)}/${getSubcategorySlug(subcategory.name, subcategory.id)}`}>{subcategory.name}</Link>
            })}
        </div>
